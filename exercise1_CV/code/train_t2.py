...
from model.model import SoftArgmax
...
SNAPSHOT_FILENAME = 'trained_net_t2.model'
VAL_MPJPE_STATE_FN = 'validation_mpjpe_state_t2'
TRAIN_MPJPE_STATE_FN = 'train_mpjpe_state_t2'
...
def create_model(file):
    if os.access(file, RW):
        ...
        model = SoftResNetModel(pretrained=True)
    model.to(device)
    return model
...
def plot(train, val):
    ...
    plt.ylim(bottom=0)
    plt.legend(loc='best')
    plt.title('Task2. MPJPE for train and validation sets over epochs.')
    plt.xlabel('epochs')
    plt.ylabel('MPJPE')
    plt.grid(True)
